...
class PoolSplitCapacityService
{
    /**
     ...
     * ]
     */
    public function validateProviderCapacity(Pool $pool, string $targetProviderType): array
    {
        $requirements = $this->calculateRequirements($pool);
     ...
     */
    private function calculateRequirements(Pool $pool): array
    {
        $inboxesPerDomain = (int) ($pool->inboxes_per_domain ?? 0);
        
        // Fix the domains extraction - remove the $order reference
        $domains = $pool ? array_map(function($domain) {
            return $domain['name'] ?? '';
        }, $pool->domains ?? []) : [];

        $domains = array_filter($domains);
        $totalFromDomains = ($inboxesPerDomain > 0 && count($domains) > 0) ? count($domains) * $inboxesPerDomain : 0;
        $fallbackTotal = (int) ($pool->total_inboxes ?? 0);
        $totalInboxes = $totalFromDomains > 0 ? $totalFromDomains : $fallbackTotal;

        if ($totalInboxes <= 0 || $inboxesPerDomain <= 0) {
            Log::warning('Pool inbox data is incomplete; cannot verify capacity', [
                'pool_id' => $pool->id, // Changed from order_id
            ...
            return [
                'success' => false,
                'message' => 'Pool inbox data is incomplete; cannot verify capacity for provider change.',
            ];
        }
    ...
    {
        $pool_panels = PoolPanel::where('is_active', 1)
