...
class PoolDomainController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($allData)
                ...
                ->addColumn('actions', function ($row) {
                    ...
                    $prefixKey = $row['prefix_key'] ?? ''; // Include prefix_key for individual edits
        ...
        return view('contractor.pool_domains.index');
    }
    ...
    public function refreshCache(Request $request)
    {
        ...
        return redirect()->route('contractor.pool-domains.index')
    ...
    public function update(Request $request)
    {
        ...
        $request->validate([
            ...
            'prefix_key' => 'nullable|string' // Optional: for updating a specific prefix
        ]);
        ...
        try {
            ...
            // Get prefix_key if provided (for individual prefix updates)
            $prefixKeyToUpdate = $request->prefix_key ?: null;
                ...
                if ($pool && is_array($pool->domains)) {
                    ...
                    foreach ($domains as $index => &$domain) {
                        if (isset($domain['id']) && $domain['id'] == $request->domain_id) {
                            ...
                            $updated = true;
                            \Log::info('Domain updated', ['domain' => $domain, 'prefix_key' => $prefixKeyToUpdate]);
                            break;
                        }
                    }
    ...
    public function viewPoolOrder($id)
    {
        ...
        return view('contractor.pool_orders.show', compact('poolOrder'));
    }
    ...
    {
        try {
            // Find the pool invoice by ID (contractor can download any invoice)
    ...
    public function assignToMe(Request $request)
    {
        ...
        try {
            ...
            // Set status to 'in-progress' when contractor is assigned (only if currently pending)
            ...
            \Log::info('Pool order #' . $poolOrder->id . ' assigned to contractor user #' . auth()->id() . ', status: ' . $poolOrder->status_manage_by_admin);
    ...
    public function changePoolOrderStatus(Request $request)
    {
        try {
            ...
            if ($statusValue === 'cancelled') {
                ...
                $reason = 'Contractor cancelled the order';
                $result = $cancellationService->cancelSubscription($request->order_id, $poolOrder->user_id, $reason);
                
                if ($result['success']) {
                    \Log::info('Pool order #' . $poolOrder->id . ' cancelled (status_manage_by_admin and subscription) by contractor user #' . auth()->id());
                    return response()->json([
                        'success' => true,
            ...
            // Use status_manage_by_admin column for contractor status changes
            $poolOrder->status_manage_by_admin = $statusValue;
            $poolOrder->save();

            \Log::info('Pool order #' . $poolOrder->id . ' status_manage_by_admin changed from ' . $oldStatus . ' to ' . $statusValue . ' by contractor user #' . auth()->id());
