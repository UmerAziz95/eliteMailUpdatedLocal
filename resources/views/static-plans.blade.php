    ...
    <div class="container py-3">
        ...
        <div class="col-12">
            <header class="text-center pb-5">
                ...
                <p style="font-size: 18px" class="mb-0">You've accessed a special pricing plan through a direct link.</p>
                <p style="font-size: 18px">{{ $masterPlan->external_name ?? $masterPlan->internal_name }} - <strong>{{ $masterPlan->description ?? 'Premium Plan Package' }}</strong></p>
            </header>
        </div>
                            ...
                            <div class="pricing-card h-100 {{ $plan->id === $selectedPlan->id ? '' : '' }}">
                                <div class="inner-content d-flex flex-column justify-content-between">
                                    <div>
                                        <div>
                                            ...
                                            <ul class="list-unstyled features-list">
                                                @if($plan->features && $plan->features->count() > 0)
                                                    ...
                                                    @endforeach
                                                @else
                                                    <li style="font-size: 16px; margin-bottom: 12px" class="d-flex align-items-center gap-2">
                                                        <div>
                                                            <img src="https://cdn.prod.website-files.com/68271f86a7dc3b457904455f/682b27d387eda87e2ecf8ba5_checklist%20(1).png"
                                                                width="25" alt="">
                                                        </div>
                                        ...
                                        <form method="POST" action="{{ route('customer.plans.subscribe', ['id' => $plan->id, 'encrypted' => $encrypted]) }}" class="w-100">
                                            @csrf
                                            <button type="submit" class="btn btn-primary py-2 border-0 subscribe-btn w-100 mt-4">
                                                <span class="btn-text">
...
<script>
    $(document).ready(function() {
        $('.subscribe-btn').click(function(e) {
            ...
            // Clear static plan session data first, then submit via AJAX
            $.ajax({
