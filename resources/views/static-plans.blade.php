...
@section('content')
@php
$publicPage = true; // This will hide sidebar and header
@endphp
    ...
    <div class="container py-3">
        ...
        <div class="col-12">
            <header class="text-center pb-5">
                <h1 class="text-center mb-3">Special Plan Access</h1>
                <p style="font-size: 18px" class="mb-0">You've accessed a special pricing plan through a direct link.
                </p>
                <p style="font-size: 18px">{{ $masterPlan->external_name ?? $masterPlan->internal_name }} - <strong>{{
                        $masterPlan->description ?? 'Premium Plan Package' }}</strong></p>
            </header>
        </div>
                ...
                <div id="plan-{{ $plan->id }}">
                    <div class="pricing-card h-100 {{ $plan->id === $selectedPlan->id ? '' : '' }}">
                        <div class="inner-content d-flex flex-column justify-content-between">
                            <div>
                                <div>
                                    <h4 style="font-size: 28px; font-weight: 500"
                                        class="text-white plan-name text-capitalize">
                                        {{ $plan->name }}
                                    </h4>
                                    <div class="mb-3">
                                    ...
                                    <h2 style="font-family: Space Grotesk; font-size: 56px;"
                                        class="fw-bold plan-price theme-text mb-4 d-flex gap-1 align-items-center mb-0">
                                        ${{ number_format($plan->price, 2) }}
                                        <span class="fw-light text-white mt-3 opacity-75"
                                            style="font-size: 17px; font-family: Space Grotesk;">
                                    ...
                                    <ul class="list-unstyled features-list">
                                        ...
                                        <li style="font-size: 16px; margin-bottom: 12px"
                                            ...
                                            {{ $feature->title ?? $feature->name }} {{ $feature->pivot->value ?? '' }}
                                        </li>
                                        @endforeach
                                        @else
                                        <li style="font-size: 16px; margin-bottom: 12px"
                                ...
                                <form method="POST"
                                    action="{{ route('customer.plans.subscribe', ['id' => $plan->id, 'encrypted' => $encrypted]) }}"
                                    class="w-100">
                                    @csrf
                                    <button type="submit"
                                        class="btn btn-primary py-2 border-0 subscribe-btn w-100 mt-4">
                                        <span class="btn-text">
...
<script>
    $(document).ready(function() {
        $('.subscribe-btn').click(function(e) {
            ...
            // Clear static plan session data first, then submit via AJAX request
            $.ajax({
                url: '{{ route("static-plans.clear-session") }}',
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function() {
                    // Session cleared, now submit the subscription form via AJAX
                    $.ajax({
                        url: formAction,
                        method: 'POST',
                        data: formData,
                        ...
                        success: function(response) {
                            // Check if response contains a hosted page URL
                            if (response.hosted_page_url || response.redirect_url || response.url) {
                                const redirectUrl = response.hosted_page_url || response.redirect_url || response.url;
                                window.location.href = redirectUrl;
                            } else if (response.redirect) {
                                window.location.href = response.redirect;
                            } else {
                                // Fallback: reload or redirect to dashboard
                        ...
                        error: function(xhr, status, error) {
                            ...
                            $btn.find('.btn-text').text($btn.find('.btn-text').data('original-text') || 'Select This Plan');
                ...
                error: function() {
                    // Even if session clear fails, proceed with subscription
                    $.ajax({
                        url: formAction,
                        method: 'POST',
                        data: formData,
        ...
        $('.subscribe-btn').each(function() {
            const $btn = $(this);
            $btn.find('.btn-text').data('original-text', $btn.find('.btn-text').text());
        });
    });
</script>
@endpush
